...
/**
  ...
  */
object SparkRuntimeEnvBase {
  private[spark] val LOG = LoggerFactory.getLogger(SparkRuntimeEnvBase.getClass)
  private[spark] var stopped = false
  private[spark] val properties = new Properties
  private[spark] var sparkContext: Option[SparkContext] = None
  private[spark] var streamingContext: Option[StreamingContext] = None
  private[spark] val sparkListeners = new ConcurrentLinkedQueue[SparkListener]()
  ...
  private implicit class ObjectReflectionFunctions(obj: Any) {
    ...
    def getField(fieldName: String): Option[_] = {
      try {
        ...
        case t: Throwable => {
          // just using obj without obj.toString causes some scala compilation errors...
          LOG.warn("Unable to access field {} from object {}", fieldName, obj.toString, t)
          None
        }
      }
    }
    ...
    def callMethod(methodName: String): Option[_] = {
      try {
        ...
        case t: Throwable => {
          // just using obj without obj.toString causes some scala compilation errors...
          LOG.warn("Unable to invoke method {} from object {}", methodName, obj.toString, t)
          None
        }
      }
    }
