...
import co.cask.cdap.api.spark.{JavaSparkExecutionContextBase, SparkExecutionContextBase, SparkSpecification}
...
/**
  * Implementation of [[co.cask.cdap.api.spark.JavaSparkExecutionContextBase]] that simply delegates all calls to
  * a [[co.cask.cdap.api.spark.JavaSparkExecutionContextBase]].
  */
trait DefaultJavaSparkExecutionContext extends JavaSparkExecutionContextBase {

  def sec: SparkExecutionContextBase
  ...
  override def fromDataset[K, V](datasetName: String, arguments: util.Map[String, String],
    ...
    JavaPairRDD.fromRDD(
      sec.fromDataset(SparkRuntimeEnvBase.getContext, datasetName, arguments.toMap, Option(splits).map(_.toIterable)))
  }
  ...
  override def fromDataset[K, V](namespace: String, datasetName: String, arguments: util.Map[String, String],
    ...
    implicit val vTag: ClassTag[V] = createClassTag
    JavaPairRDD.fromRDD(
      sec.fromDataset(SparkRuntimeEnvBase.getContext, namespace, datasetName, arguments.toMap,
  ...
  override def fromStream(streamName: String, startTime: Long, endTime: Long): JavaRDD[StreamEvent] = {
    ...
    JavaRDD.fromRDD(
      sec.fromStream(SparkRuntimeEnvBase.getContext, streamName, startTime, endTime)(ct, (e: StreamEvent) => e))
  }
  ...
  JavaRDD[StreamEvent] = {
    val ct: ClassTag[StreamEvent] = createClassTag
    JavaRDD.fromRDD(
      sec.fromStream(SparkRuntimeEnvBase.getContext, namespace, streamName, startTime, endTime)(ct, (e: StreamEvent) => e))
  }
  ...
  override def fromStream[T](streamName: String, formatSpec: FormatSpecification,
    ...
    val stream: RDD[(Long, GenericStreamEventData[T])] =
      sec.fromStream(SparkRuntimeEnvBase.getContext, streamName, formatSpec, startTime, endTime)
  ...
  override def fromStream[T](namespace: String, streamName: String, formatSpec: FormatSpecification,
    ...
    implicit val dTag: ClassTag[T] = ClassTag(dataType)
    val stream: RDD[(Long, GenericStreamEventData[T])] =
      sec.fromStream(SparkRuntimeEnvBase.getContext, namespace, streamName, formatSpec, startTime, endTime)
    JavaPairRDD.fromRDD(stream.map(t => (t._1: java.lang.Long, t._2)))
  }
  ...
  private def decodeFromStream[K: ClassTag, V: ClassTag](streamName: String, startTime: Long, endTime: Long,
    ...
    sec.fromStream(SparkRuntimeEnvBase.getContext, streamName, startTime, endTime)(ct, identity)
  ...
  private def decodeFromStream[K: ClassTag, V: ClassTag](namespace: String, streamName: String,
                                                        ...
                                                        (implicit ct: ClassTag[StreamEvent]): RDD[(K, V)] = {
    val identity = (e: StreamEvent) => e
    sec.fromStream(SparkRuntimeEnvBase.getContext, namespace, streamName, startTime, endTime)(ct, identity)
      .mapPartitions(createStreamMap(decoderClass))
  }
