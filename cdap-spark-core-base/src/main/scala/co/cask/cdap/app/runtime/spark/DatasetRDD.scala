...
/**
  ...
  */
abstract class DatasetRDD[K: ClassTag, V: ClassTag](@transient sc: SparkContext,
  ...
  def getInputFormatClass(inputFormatClassName: String): Class[InputFormat[K, V]]

  def getBatchReadableRDD(batchReadable: BatchReadable[K, V]): RDD[(K, V)]
      ...
      delegateRDD = Some(datasetCompute(namespace, datasetName, arguments, (dataset: Dataset) => {
        ...
        dataset match {
          case batchReadable: BatchReadable[K, V] => {
            getBatchReadableRDD(batchReadable)
          }
          ...
          case inputFormatProvider: InputFormatProvider => {
            ...
            val inputFormatClass = getInputFormatClass(inputFormatClassName)
