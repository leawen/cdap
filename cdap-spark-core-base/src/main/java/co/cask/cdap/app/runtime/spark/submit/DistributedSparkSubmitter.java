...
import co.cask.cdap.app.runtime.spark.SparkRuntimeEnvBase;
...
public class DistributedSparkSubmitter extends AbstractSparkSubmitter {
  ...
  public DistributedSparkSubmitter(Configuration hConf, LocationFactory locationFactory,
    ...
    super(sparkRuntimeInfoProvider);
    this.hConf = hConf;
  ...
  protected List<String> beforeSubmit() {
    ...
    for (Map.Entry<String, String> entry : hConf) {
      SparkRuntimeEnvBase.setProperty("spark.hadoop." + entry.getKey(), hConf.get(entry.getKey()));
    }

    sparkExecutionService.startAndWait();
    return Collections.singletonList("--" + SparkMainWrapperState.ARG_EXECUTION_SERVICE_URI()
