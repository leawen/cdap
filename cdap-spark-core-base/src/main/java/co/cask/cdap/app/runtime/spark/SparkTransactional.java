...
final class SparkTransactional implements Transactional {
  ...
  enum TransactionType {
     ...
     * SparkExecutionContext#saveAsDataset(RDD, String, scala.collection.immutable.Map, ClassTag, ClassTag)
     */
    IMPLICIT,

    /**
     * Implicit transaction with commit on job end, initiated by
     * SparkExecutionContext#fromDataset(SparkContext, String, scala.collection.immutable.Map,
  ...
  private final ThreadLocal<TransactionalDatasetContext> activeDatasetContext =
    new ThreadLocal<TransactionalDatasetContext>() {
      ...
      public void set(TransactionalDatasetContext value) {
        ...
        if (SparkRuntimeEnvBase.setLocalProperty(ACTIVE_TRANSACTION_KEY, txKey)) {
      ...
      @Override
      public void remove() {
        String txKey = SparkRuntimeEnvBase.getLocalProperty(ACTIVE_TRANSACTION_KEY);
        if (txKey != null && !txKey.isEmpty()) {
          // Spark doesn't support unsetting of property. Hence set it to empty.
          SparkRuntimeEnvBase.setLocalProperty(ACTIVE_TRANSACTION_KEY, "");
