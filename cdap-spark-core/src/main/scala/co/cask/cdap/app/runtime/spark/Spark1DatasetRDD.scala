...
/**
  ...
  */
class Spark1DatasetRDD[K: ClassTag, V: ClassTag](@transient sc: SparkContext,
  ...
  override def getInputFormatClass(inputFormatClassName: String): Class[InputFormat[K, V]] = {
    Spark1ClassLoader.findFromContext()
  ...
  override def getBatchReadableRDD(batchReadable: BatchReadable[K, V]): RDD[(K, V)] = {
    new Spark1BatchReadableRDD[K, V](sc, batchReadable, namespace, datasetName, arguments, splits, txServiceBaseURI)
  }
}
