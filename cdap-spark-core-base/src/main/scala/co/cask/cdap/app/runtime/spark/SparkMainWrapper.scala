 ...
 *
 * @tparam T type of SparkExecutionContext stored in the SparkClassLoader
 */
trait SparkMainWrapper[T] {

  // Import the companion object for static fields
  import SparkMainWrapper._
  import SparkMainWrapperState._

  def runSparkProgram(userSparkClass: Class[_], sparkClassLoader: SparkClassLoader[T],
                      runtimeContext: SparkRuntimeContext): Unit

  def getSparkClassLoader(): SparkClassLoader[T]
  ...
  def run(args: Array[String]): Unit = {
    ...
    val sparkClassLoader = getSparkClassLoader()
    ...
    try {
      ...
      try {
        ...
        try {
          runSparkProgram(userSparkClass, sparkClassLoader, runtimeContext)
          stopped = true
        } finally {
  ...
  protected def getMainMethod(obj: Class[_]): Option[Method] = {
    try {
  ...
  private def startHeartbeat(arguments: Map[String, String],
    ...
    arguments.get(ARG_EXECUTION_SERVICE_URI).fold(new Cancellable {
      ...
      if (!SparkRuntimeEnvBase.isStopped) {
  ...
  private def createCredentialsUpdater(hConf: Configuration,
    ...
    try {
      ...
      SparkRuntimeEnvBase.setProperty("spark.yarn.credentials.file", credentialsFile.toURI.toString)
...
/**
  * State used by SparkMainWrapper.
  */
object SparkMainWrapperState {
  val ARG_USER_CLASS = "userClass"
  val ARG_EXECUTION_SERVICE_URI = "executionServiceURI"

  @volatile
  private[spark] var stopped = false

  @volatile
  private[spark] var mainThread: Option[Thread] = None
  ...
  def triggerShutdown(): Unit = {
    stopped = true
    SparkRuntimeEnvBase.stop(mainThread)
  }
}
